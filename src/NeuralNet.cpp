...
NeuralNet::NeuralNet (int input_size, int hidden_layers, int hidden_neurons, int output_size, double learn_rate) {
    lr_ = learn_rate;
...
void NeuralNet::train(const std::vector<double>& input, const std::vector<double>& target) {
    ...
    for (int l = layers_.size() - 2; l >= 0; l--) {
        ...
        for (size_t i = 0; i < current.neurons_.size(); i++) {
            ...
            current.neurons_[i].delta_ = error * dsigmoid(current.neurons_[i].output_);
        }
    }
