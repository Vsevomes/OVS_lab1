...
int main(int argc, char* argv[]) {
  ...
  if (mode == "train") {
    ...
    auto duration_train =
        std::chrono::duration_cast<ms>(end_train - start_train).count();

    auto start_forward = Clock::now();
    auto out = net.forward(samples[0].input);
    auto end_forward = Clock::now();
    auto duration_forward =
        std::chrono::duration_cast<mc>(end_forward - start_forward).count();
    ...
    std::cout << "Training completed for " << epoch << " epochs, ERR = " << err
              << std::endl;
    ...
    std::vector<std::vector<int>> confusion(num_outputs,
                                            std::vector<int>(num_outputs, 0));
    ...
    for (auto& i : inputs) {
      ...
      int actual =
          std::max_element(i.target.begin(), i.target.end()) - i.target.begin();
      ...
      if (predicted == 0) {
        ...
        std::cout << "Shape: rectangle (p=" << out[predicted] << ")"
                  << std::endl;
      } else if (predicted == 2) {
        std::cout << "Shape: triangle (p=" << out[predicted] << ")"
                  << std::endl;
      } else {
    ...
    for (int c = 0; c < num_outputs; c++) {
      ...
      std::cout << "Class " << c << " -> Precision: " << precision
                << ", Recall: " << recall << ", F1-score: " << f1 << std::endl;
    }
  } else {
...
void menu(std::string prog_name) {
  ...
  std::cerr << "  For train model: " << prog_name
            << " train <train_folder> <inputs> <hiddenLayers> "
               "<neuronsPerLayer> <outputs>"
            << std::endl;
  std::cerr << "  For use model: " << prog_name
            << " predict <model_file> <input_folder>" << std::endl;
}
